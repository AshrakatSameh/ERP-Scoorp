...
export class TypeEmployeeRequestsComponent implements OnInit {
  ...
  onSubmitAdd(): void {
    ...
    // Append approval levels
    this.ApprovalLevels.controls.forEach((levelControl, levelIndex) => {
      const level = levelControl.get('Level')?.value;
  
      // Append the level value
      formData.append(`approvalLevels[${levelIndex}].level`, level);
  
      // Get the approverUserIds from the control
      const approverUserIds = levelControl.get('approverUserIds')?.value || [];
  
      // Append each approverUserId to the respective approval level
      approverUserIds.forEach((userId: string, userIdIndex: number) => {
        formData.append(`approvalLevels[${levelIndex}].userIds[${userIdIndex}]`, userId);
      });
    });
